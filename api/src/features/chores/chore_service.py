...
def create_and_get_chores_for_date(date: datetime, child_id: int):
  ...
  scheduled_chores = chore_repository.get_chores_scheduled_for_day_of_week(day_of_week, child_id)
  chores: List[ChildChore] = []
  for chore in scheduled_chores:
    child_chore = chore_repository.get_chore_scheduled_for_day(date, child_id, chore.id)
    if child_chore:
